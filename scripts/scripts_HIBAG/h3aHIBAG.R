...
# save(mobj, file="h3a_HLA_A_Model.RData")
...
hlaParallelAttrBagging(cl, train.HLA_A, train.geno, nclassifier=100, 
                      auto.save = "h3a_HLA_A_Model.RData", stop.cluster=TRUE)
model.obj <- get(load("h3a_HLA_A_Model.RData"))
...
# mobj <- hlaPublish(model,
#                    platform = "Illumina 1M Duo",
#                    information = "Training set -- 1000 genomes, h3a",
#                    warning = NULL,
#                    rm.unused.snp=TRUE, anonymize=TRUE)

# save(mobj, file="h3a_HLA_B_Model.RData")
...
hlaParallelAttrBagging(cl, train.HLA_B, train.geno, nclassifier=100, 
                      auto.save = "h3a_HLA_B_Model.RData", stop.cluster=TRUE)
model.obj <- get(load("h3a_HLA_B_Model.RData"))
model <- hlaModelFromObj(model.obj)
summary(model)
...
# save(mobj, file="h3a_HLA_C_Model.RData")
...
hlaParallelAttrBagging(cl, train.HLA_C, train.geno, nclassifier=100, 
                      auto.save = "h3a_HLA_C_Model.RData", stop.cluster=TRUE)
model.obj <- get(load("h3a_HLA_C_Model.RData"))
model <- hlaModelFromObj(model.obj)
summary(model)
