...
#STEP 10
#EVALUATE OVERALL ACCURACY SENSITIVITY AND SPECIFICITY
#Split your data into training and validation
set.seed(100)
hlatab <- hlaSplitAllele(train.HLA_A, train.prop=0.5)
names(hlatab)
...
test.geno <- hlaGenoSubset(geno, samp.sel=match(
  hlatab\$validation\$value\$sample.id, geno\$sample.id))

#Train a HIBAG model using the training population
set.seed(100)
model <- hlaAttrBagging(hlatab\$training, train.geno, nclassifier=100)
summary(model)

#Validate your test genotype using the model
...
#Split your data into training and validation
set.seed(100)
hlatab <- hlaSplitAllele(train.HLA_B, train.prop=0.5)
names(hlatab)
summary(hlatab\$training)
summary(hlatab\$validation)
...
#Split your data into training and validation
set.seed(100)
hlatab <- hlaSplitAllele(train.HLA_C, train.prop=0.5)
names(hlatab)
summary(hlatab\$training)
summary(hlatab\$validation)
