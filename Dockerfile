...
RUN git clone https://github.com/nanjalaruth/MHC-Imputation-Accuracy.git && \
  ...
  chmod -R a+rwx /usr/local/bin/MakeReference && \
  chmod -R a+rwx /usr/local/bin/SNP2HLA && \  
...
RUN wget http://zzz.bwh.harvard.edu/plink/dist/plink-1.07-x86_64.zip && \
  ...
  cp beagle.jar ./MakeReference && \
  cp beagle.jar ./SNP2HLA && \
  rm -fr beagle_3.0.4_05May09.zip beagle.3.0.4 beagle.jar && \
