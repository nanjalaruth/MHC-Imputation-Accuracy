...
RUN git clone https://github.com/WansonChoi/CookHLA.git && \
  ...
  # mv -f * /usr/local/bin && \
  chmod -R a+rwx CookHLA.py && \ 
  chmod -R a+rwx CookHLA_lab.py && \
  chmod -R a+rwx CookHLA_lab_bglv5.py && \
  chmod -R a+rwx MakeGeneticMap/* && \
  chmod -R a+rwx measureAcc/__init__.py && \
  chmod -R a+rwx measureAcc/__main__.py && \
  chmod -R a+rwx measureAcc/measureAccuracy.py && \
  chmod -R a+rwx measureAcc/NomenCleaner/* && \
  chmod -R a+rwx measureAcc/src/* && \
  chmod -R a+rwx measureAcc/data/* && \
  chmod -R a+rwx src/* && \
  mv -f * .. && \
  cd .. && \
  rm -fr CookHLA
